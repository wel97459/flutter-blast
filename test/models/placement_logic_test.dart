...
void main() {
  group('Placement Logic', () {
    ...
    setUp(() {
      emptyGrid = List.generate(8, (_) => List.generate(8, (_) => Block.empty()));
    });
    ...
    test('should return false for out of bounds placement', () {
      final piece = Piece(shape: [(0,0), (0,1)], color: 0xFF0000FF);
      expect(isValidPlacement(emptyGrid, piece, 0, 7), isFalse); // One block out of bounds (col 8)
      expect(isValidPlacement(emptyGrid, piece, 8, 0), isFalse); // Out of bounds row
      expect(isValidPlacement(emptyGrid, piece, -1, 0), isFalse);
    });

    test('should return false for overlapping blocks', () {
      final grid = List.generate(8, (_) => List.generate(8, (_) => Block.empty()));
    ...
    group('Score Calculation', () {
      test('should calculate score for piece placement', () {
        final piece = Piece(shape: [(0,0), (0,1), (1,1)], color: 0xFF0000FF);
        expect(R(piece, 0), 3); // 3 blocks * 10
      });

      test('should calculate score for piece placement and line clears', () {
        final piece = Piece(shape: [(0,0), (0,1)], color: 0xFF0000FF);
        expect(calculateScore(piece, 1), 101); // 2 blocks * 1 + 1 line * 100
        expect(calculateScore(piece, 2), 202); // 2 blocks * 1 + 2 lines * 100
      });
    });
    ...
    group('Game Over Condition', () {
      test('should NOT be game over if at least one piece can be placed', () {
        ...
        final piece1 = Piece(shape: [(0,0), (0,1), (0,2)], color: 0xFF0000FF); // 1x3 horizontal
        final piece2 = Piece(shape: [(0,0), (1,0)], color: 0xFF0000FF); // 2x1 vertical
      ...
      test('should be game over if no pieces can be placed', () {
        final grid = List.generate(8, (_) => List.generate(8, (_) => const Block(color: 0xFF00FF00)));
        // Grid is full
      ...
      test('should be game over if pieces are too large for remaining gaps', () {
        final grid = List.generate(8, (_) => List.generate(8, (_) => const Block(color: 0xFF00FF00)));
        grid[0][0] = Block.empty();
        grid[7][7] = Block.empty();
        // Only two isolated empty cells.
        
        final piece = Piece(shape: [(0,0), (0,1)], color: 0xFF0000FF); // 1x2 piece
        expect(isGameOver(grid, [piece]), isTrue);
      });
    });
  });
